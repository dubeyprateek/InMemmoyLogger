...
namespace winrt::InMemmoyLogger::implementation
{
    ...
    private:
        ...
        VOID Write(LONG Index, const PVOID& currentMemoryLocation, const WCHAR*  messageBuffer);
        VOID InitializeLogMemory(hstring const& name);

    private:
        std::atomic<LONG>   countCircularBuffer                         ;
        ...
        BOOL                isInitialized                               ;
        PVOID               circularLogIndex[NUM_BUFFER]                ;
        PVOID               persistentLogIndex[NUM_BUFFER]              ;
